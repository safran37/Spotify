...
export default function Songitem({item}) {
  ...
  return (
    ...
    <div className='pt-[100%] relative mb-4'>
      <img className={`${imageStyle(item)} absolute object-cover inset-0 w-full h-full`} src={item.img}></img>
      <button onClick={updateCurrent} className='w-10 h-10 group-hover:flex group-focus:flex hidden items-center justify-center rounded-full bg-primary absolute bottom-2 right-2'>
        {current?.id == item.id && playing ? <BsFillPauseFill size={20}/> : <BsFillPlayFill size={20}/>}
      </button>
    </div>
